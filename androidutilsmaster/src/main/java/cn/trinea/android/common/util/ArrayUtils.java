 ...
 * </ul>
 * 
 * isEmpty(V[] sourceArray) 判断数组是否为空或长度为0 getLast(V[] sourceArray, V value, V
 * defaultValue, boolean isCircle) 得到数组中某个元素前一个元素，isCircle表示是否循环 getNext(V[]
 * sourceArray, V value, V defaultValue, boolean isCircle)
 * 得到数组中某个元素下一个元素，isCircle表示是否循环 源码可见ArrayUtils.java，更多方法及更详细参数介绍可见ArrayUtils
 * Api Guide。
 */
