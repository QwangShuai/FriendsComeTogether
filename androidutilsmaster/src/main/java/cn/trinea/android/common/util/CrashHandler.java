...
public class CrashHandler implements UncaughtExceptionHandler {
	 ...
	 * @param context
	 */
	public void init(Context context,String logPath,onCrash lis) {
		this.lis= lis;
		mContext = context;
		this.path = logPath;
	...
	private void sendErrorMail(Context _context , String filePath) {
		...
		sendIntent.putExtra(Intent.EXTRA_EMAIL,new String[] {"594030861@qq.com"});
		sendIntent.putExtra(Intent.EXTRA_TEXT, body);
		sendIntent.putExtra(Intent.EXTRA_SUBJECT, subject);
		sendIntent.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(new File(filePath)) );
		sendIntent.setType("message/rfc822");
		_context.startActivity(Intent.createChooser(sendIntent, "拾忆"));
	}
	/**
	 * 崩溃接口
	 * @author slg
	...
	public interface onCrash{
		public void onCrashLis(String error);
	}
