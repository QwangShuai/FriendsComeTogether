 ...
 * </ul>
 * 
 * Android系统下载管理DownloadManager增强方法，可用于包括获取下载相关信息，如： getStatusById(long) 得到下载状态
 * getDownloadBytes(long) 得到下载进度信息 getBytesAndStatus(long) 得到下载进度信息和状态
 * getFileName(long) 得到下载文件路径 getUri(long) 得到下载uri getReason(long) 得到下载失败或暂停原因
 * getPausedReason(long) 得到下载暂停原因 getErrorCode(long) 得到下载错误码
 * 源码可见DownloadManagerPro.java，更多方法及更详细参数介绍可见DownloadManagerPro Api
 * Guide。关于Android DownManager使用可见DownManager Demo。
 */
