 ...
 * </ul>
 *
 * Android包相关工具类，可用于(root)安装应用、(root)卸载应用、判断是否系统应用等，如： install(Context, String)
 * 安装应用，如果是系统应用或已经root，则静默安装，否则一般安装 uninstall(Context, String)
 * 卸载应用，如果是系统应用或已经root，则静默卸载，否则一般卸载 isSystemApplication(Context, String)
 * 判断应用是否为系统应用 源码可见PackageUtils.java，更多方法及更详细参数介绍可见ShellUtils Api
 * Guide。关于静默安装可见apk-root权限静默安
 */
